...
const SITE_URL = "https://bz-stport-bazar.vercel.app";
...
export function ProductCard({ product, soldOut, adminMode, onToggleSold }: Props) {
  ...
  return (
    <Card
      className={cn(
        "group overflow-hidden border-border/50 bg-card transition-all duration-300",
        soldOut
          ? "opacity-60 grayscale"
          : "hover:border-primary/30 hover:shadow-[0_0_30px_-5px] hover:shadow-primary/20",
        adminMode && "cursor-pointer ring-2 ring-transparent hover:ring-amber-500/50"
      )}
      onClick={adminMode ? () => onToggleSold?.(product.id) : undefined}
    >
      <div className="relative aspect-[4/3] overflow-hidden bg-muted">
        ...
        {soldOut && (
          <div className="absolute inset-0 z-20 flex items-center justify-center bg-black/60">
            <span className="text-white font-bold text-lg sm:text-xl tracking-wider rotate-[-15deg] border-2 border-white px-4 py-1 rounded">
              VENDIDO
            </span>
          </div>
        )}
        ...
        <div className="flex items-end justify-between">
          ...
          {soldOut ? (
            <span className={cn(
              ...
              "bg-gray-600 text-gray-300 cursor-not-allowed pointer-events-none text-xs"
            )}>
              Indisponivel
            </span>
          ) : (
